...
class TpaBookingPage extends BasePage {
  constructor(page) {
    super(page);
    ...
    this.bookingType = page.getByRole('radio', { name: 'TPA' });
    ...
    this.tpaProfileArrow = page.locator('div:nth-child(10) > .ng-select-searchable > .ng-select-container > .ng-arrow-wrapper');
    this.voucherInput = page.getByRole('textbox', { name: 'Enter Voucher' });
    this.otaNameArrow = page.locator('div:nth-child(12) > .ng-select-searchable > .ng-select-container > .ng-arrow-wrapper');
  ...
  async openBooking() {
    ...
    await this.bookBtn.waitFor({ state: 'visible', timeout: 15000 });
  ...
  async enterTpaDetails(profile, voucher, ota) {
    await this.tpaProfileArrow.waitFor({ state: 'visible', timeout: 5000 });
    await this.tpaProfileArrow.click();
    await this.page.getByRole('option', { name: profile }).click();
    await this.voucherInput.click();
    await this.voucherInput.fill(voucher);
    await this.otaNameArrow.click();
    await this.page.getByRole('option', { name: ota }).click();
  }
  ...
  async enterLocation(query, resultText) {
    ...
    const result = this.locationResultItem(resultText).first();
